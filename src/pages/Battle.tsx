...
const Battle = () => {
  ...
  const handleSelectCharacter = (character: CryptoCharacter, isPlayer: boolean) => {
    if (isPlayer) {
      ...
      toast({
        ...
        description: `Вы выбрали ${character.symbol} для сражения`,
        duration: 2000,
      });
    } else {
      setOpponentCharacter(character);
      toast({
        title: `${character.name} будет вашим противником!`,
        description: `${character.symbol} готовится к битве`,
        duration: 2000,
      });
    }
  ...
  const startBattle = () => {
    ...
    setTimeout(() => {
      ...
      if (result.winner) {
        toast({
          title: `${result.winner.name} одержал победу!`,
          description: `Счет: ${result.score.player} против ${result.score.opponent}`,
          duration: 5000,
        });
      } else {
        toast({
        ...
        <div className="flex flex-col items-center justify-center space-y-8">
          {battleStatus === "selecting" && (
            <div className="w-full max-w-3xl text-center space-y-6">
              <h2 className="text-xl md:text-2xl font-bold">Выберите двух персонажей для битвы</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <h3 className="font-medium text-lg">Противник</h3>
                  <div className="border-2 border-dashed border-game-secondary/30 rounded-xl p-6 min-h-[300px] flex items-center justify-center">
                    {opponentCharacter ? (
                      ...
                      <div className="flex flex-col items-center gap-4">
                        <p className="text-muted-foreground">Выберите противника из списка ниже</p>
            ...
            <div className="w-full max-w-4xl animate-slide-up space-y-8">
              <div className="flex flex-col items-center gap-4">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-center">
                  {battleResult.winner 
                    ? `${battleResult.winner.name} одержал победу!` 
                    : "Ничья!"}
                </h2>
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-8">
                ...
                <div className="relative">
                  ...
                  />
                  <div className="absolute top-3 right-3 bg-background/80 backdrop-blur-sm rounded-full px-3 py-1 font-bold border">
                    {battleResult.score.opponent} очков
                  </div>
                </div>
              </div>
            </div>
